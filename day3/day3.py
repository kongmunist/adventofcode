...
# Solving part 1
# Split each element of input into list of 1s and 0s
for i in range(len(inputs)):
    inputs[i] = [int(x) for x in list(inputs[i])]

# Find the most common bit in each position
gamma_rate = []
epsilon_rate = []
for i in range(len(inputs[0])):
    # iterate over all elements in the ith postion of each input
    ...
    for j in range(len(inputs)):
        if inputs[j][i] == 1:
            ...
            zeros += 1
    # if there are more 1s than 0s, the most common bit is 1
    if ones > zeros:
        gamma_rate.append(1)
...
# convert gamma_rate and epsilon_rate from binary to ints
